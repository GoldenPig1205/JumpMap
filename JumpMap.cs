...
namespace JumpMap
{
    public class Hit
    {
        ...
        public void Update()
        {
            ...
            if (Movestate != _Movestate)
            {
                ...
                {
                    JumpMap.Instance.OnMelee(player);
                    CanHit = false;
        ...
        void Check(Player player)
        {
            ...
            if (Physics.Raycast(player.Position, Vector3.down, out RaycastHit hit, 1, (LayerMask)1))
            {
                ...
                if (pos.StartsWith("Stage "))
                {
                    ...
                    int ul = int.Parse(JumpMap.Instance.Stage[player.UserId]);

                    if (level > ul)
                        JumpMap.Instance.Stage[player.UserId] = level.ToString();
                }
    ...
    public class JumpMap : Plugin<Config>
    {
